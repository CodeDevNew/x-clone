...
export const createComment = async (req , res)=>{
    try {
        const {text} = req.body;
        const postId = req.params.id;// not destructuring method ,it is assign variable method
...
export const likeUnlikePost = async (req, res) => {
    try {
        ...
        if (userLikedPost) {
            ...
            // Create a notification (optional, no response sent here)
            ...
            return res.status(200).json(updatedLikes); // Ensure a single response
        }
    } catch (error) {
        console.error(`Error in like and unlike controller: ${error}`);
